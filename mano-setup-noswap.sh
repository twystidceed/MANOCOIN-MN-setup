#!/bin/bash
# MANOCOIN Masternode Setup Script V1.0 for Ubuntu 16.04 LTS
#
# Script will attempt to auto detect primary public IP address
# and generate masternode private key unless specified in command line
#
# Usage:
# bash mano-setup-noswap.sh 
#
...
echo -e "${YELLOW}MANO Masternode Setup Script V1 for Ubuntu 16.04 LTS${NC}"
echo "Do you want me to generate a masternode private key for you? [y/n]"
  read DOSETUP
if [[ $DOSETUP =~ "n" ]] ; then
          read -e -p "Enter your Masternode private key:" genkey;
...
#Check Deps
sudo apt-get install -y unzip dtrx
...
#Generating Random Password for LeisureCoind JSON RPC
rpcuser=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)
rpcpassword=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)

 #Extracting Daemon
dtrx -n -f MANO_Ubuntu_16_04.tar.gz
rm -rf MANO_Ubuntu_16_04.tar.gz
rm mano-setup-noswap.sh
rm mano-setup.sh
 
 stop_daemon
 
 # Deploy binaries to /usr/bin
 sudo cp mano* /usr/bin/
 sudo chmod 755 -R ~/MANOCOIN-MN-setup
 sudo chmod 755 /usr/bin/mano*
 
 # Deploy masternode monitoring script
 cp ~/MANOCOIN-MN-setup/manomon.sh /usr/local/bin/
 sudo chmod 711 /usr/local/bin/manomon.sh
 
 #Create datadir
 if [ ! -f ~/.manocoin/mano.conf ]; then 
 	sudo mkdir ~/.manocoin
 fi
...
if [ -z $genkey ]; then
    cat <<EOF > ~/.manocoin/mano.conf
rpcuser=$rpcuser
rpcpassword=$rpcpassword
EOF

    sudo chmod 755 -R ~/.manocoin/mano.conf
...
#Finally, starting daemon with new mano.conf
manod -daemon
delay 10
...
Currently your masternode is syncing with the LSR network...
